...
RSpec.describe Dbml2Mmd::CLI do
  ...
  before do
    allow(Slop::Options).to receive(:new).and.return(mock_parser)
    allow(mock_parser).to receive(:banner=)
    allow(mock_parser).to receive(:on)
    allow(mock_parser).to receive(:parse).and.return(mock_opts)
    allow(File).to receive(:read).and.return(simple_dbml)
    allow(File).to receive(:write)
    allow(File).to receive(:exist?).and.return(true)
  end
  ...
  describe '#run' do
    context 'when --help flag is provided' do
      ...
      before do
        allow(mock_opts).to receive(:help?).and.return(true)
        allow(mock_parser).to receive(:to_s).and.return('Usage help')
      end

      it 'displays the help message' do
        expect { cli.run }.to output(/Usage help/).to_stdout
      end
    ...
    context 'when --version flag is provided' do
      ...
      before do
        allow(mock_opts).to receive(:help?).and.return(false)
        allow(mock_opts).to receive(:version?).and.return(true)
      end
    ...
    context 'when processing an input file' do
      before do
        allow(mock_opts).to receive(:help?).and.return(false)
        allow(mock_opts).to receive(:version?).and.return(false)
        allow(mock_opts).to receive(:[]).with(:output).and.return(nil)
        allow(mock_opts).to receive(:[]).with(:theme).and.return(nil)
        allow(mock_opts).to receive(:args).and.return(args)
      end
    ...
    context 'when output file option is provided' do
      ...
      before do
        allow(mock_opts).to receive(:help?).and.return(false)
        allow(mock_opts).to receive(:version?).and.return(false)
        allow(mock_opts).to receive(:[]).with(:output).and.return('output.mmd')
        allow(mock_opts).to receive(:[]).with(:theme).and.return(nil)
        allow(mock_opts).to receive(:args).and.return(args)
      end
    ...
    context 'when theme option is provided' do
      ...
      before do
        ...
        allow(mock_opts).to receive(:version?).and.return(false)
        allow(mock_opts).to receive(:[]).with(:output).and.return(nil)
        allow(mock_opts).to receive(:[]).with(:theme).and.return('dark')
        allow(mock_opts).to receive(:args).and.return(args)
      end
    ...
    context 'when an invalid file is provided' do
      ...
      it 'displays an error message and exits with status 1' do
        expect { cli.run }.to output(/Error: File not found/).to_stdout.and raise_error(SystemExit) { |error|
